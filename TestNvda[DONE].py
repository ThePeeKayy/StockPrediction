...
model = QRDQN('MlpPolicy', env, buffer_size=32000, learning_rate=0.0002, verbose=1, device=device)
model.learn(total_timesteps=300000)  # Adjusted timesteps
model.save("Nvidia_QRDQN_stock_trading")

# Load the model
model = QRDQN.load("Nvidia_QRDQN_stock_trading", env=env, device=device)
...
print(f"Nvda Total Reward: {rewards}")
