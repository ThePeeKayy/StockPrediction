...
# model = QRDQN('MlpPolicy', env, buffer_size=20000, learning_rate=0.0003, verbose=1, device=device) # was 15000 buffer size
# model.learn(total_timesteps=300000)
# model.save(f"BRKB_QRDQN_stock_trading")

# Load the models
model = QRDQN.load(f"BRKB_QRDQN_stock_trading", env=env, device=device) 
...
optimizer = optim.Adam(q_network.parameters(), lr=0.007)
loss_fn = nn.MSELoss()

# Define hyperparameters
gamma = 0.99
epsilon = 1.0
epsilon_decay = 0.95  # 0.997
min_epsilon = 0.01 
batch_size = 128
...
for episode in range(num_episodes):
    ...
    random_steps = random.randint(0, len(env.df) - 2350)
    ...
    total_reward = 0
    step_count = 0
    while not done and step_count < 2300:
...
print(f"BRK.B Total Reward: {rewards}")
...
buy_and_hold_values = [initial_price]
for return_value in cumulative_returns[1:]:
    buy_and_hold_values.append(initial_price * return_value)
