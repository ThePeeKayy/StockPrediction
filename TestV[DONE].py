...
# Download stock data
df = pd.read_csv('bigV.csv')
test_df = df[:2200]
df = df[-2200:].reset_index(drop=True)
...
model = QRDQN('MlpPolicy', env, buffer_size=29000, learning_rate=0.0003, verbose=1, device=device)

# Train the model
model.learn(total_timesteps=410000)
model.save("Visa_QRDQN_stock_trading")

# Load the model
model = QRDQN.load("Visa_QRDQN_stock_trading", env=env, device=device)
...
print(f"Visa Total Reward: {rewards}")
test_df = test_df[:2101]
...
plt.plot(test_df.index, buy_and_hold_values[:2101], color='blue', label='Buy and Hold Strategy', linestyle='--')
