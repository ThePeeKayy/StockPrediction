...
def main():
    stock_data = pd.read_csv('bigAAPL.csv', parse_dates=['Date'])
    ...
    # Splitting the data
    train_data = merged_data[-2000:-130].reset_index(drop=True)
    val_data = merged_data[-2000:-65].reset_index(drop=True)
    test_data = merged_data[-430:].reset_index(drop=True)

    # Fit and transform the feature columns in the training data
    train_data[feature_columns] = feature_scaler.fit_transform(train_data[feature_columns])

    # Fit and transform the target value in the training data
    train_data['value'] = value_scaler.fit_transform(train_data[['value']])

    # Now, transform (without fitting) the validation and test data using the scalers fitted on train data
    val_data[feature_columns] = feature_scaler.transform(val_data[feature_columns])
    val_data['value'] = value_scaler.transform(val_data[['value']])

    test_data[feature_columns] = feature_scaler.transform(test_data[feature_columns])
    test_data['value'] = value_scaler.transform(test_data[['value']])
    ...
    # val_dataloader = validation.to_dataloader(train=False, batch_size=160, num_workers=8)
    ...
    #     dirpath='./TFTfiles',
    #     filename='{epoch}-{val_MAE:.4f}-AAPL_unks',
    #     save_top_k=2,
    #     monitor='val_MAE',
    #     mode='min'
    # )
    ...
    tft = TemporalFusionTransformer.load_from_checkpoint("./TFTfiles/epoch=8-val_MAE=3.011-AAPL_unks.ckpt")
    ...
    actual_values = value_scaler.inverse_transform(test_data['value'].iloc[max_encoder_length:max_encoder_length + max_prediction_length].values.reshape(-1, 1)).flatten()
